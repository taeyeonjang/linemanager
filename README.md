...
현장에서 쌓은 도메인 지식을 바탕으로 실제 공장 라인의 동작 방식을 최대한 유사하게 재현하였으며, 비동기 처리, 동적 쿼리, 계층형 아키텍처 등 백엔드 핵심 개념을 직접 적용했습니다.
...
- **오토/수동 모드**: 오토 모드 시 대기 큐의 제품을 자동으로 연속 처리
...
3번 공정에서 NG 발생 후 ROS에서 NG 판정 시, 4번 공정의 NG 배출구로 이동하는 것을 표현해야 했습니다.

처음에는 ProcessLog(4번)를 생성했지만, 실제로는 4번 공정 검사를 수행하지 않았기 때문에 이력에 남기는 것이 도메인적으로 잘못되었습니다.
